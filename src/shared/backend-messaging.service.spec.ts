...
describe('Backend Messaging Service tests', () => {

  const baseUrl = 'http://test.dev';

  let apiServiceFactory = (http: Http, token: TokenService) => {
    return new BackendMessagingService(baseUrl, http, token);
  };
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        {provide: BackendMessagingService, useFactory: apiServiceFactory, deps: [Http, TokenService]}
      ]
    });
  });

  it('should use full url',
    inject([MockBackend, BackendMessagingService], (mockBackend: MockBackend, apiService: BackendMessagingService) => {
